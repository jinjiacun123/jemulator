...
int res_w = 320, res_h = 240;

void m_renderer_update_display_area(m_simplestation_state *m_simplestation)//[by jim] need
{
}

void draw(m_simplestation_state *m_simplestation, bool clear_colour, bool part, bool isline)//[by jim] need
{
...
void m_sync_vram(m_simplestation_state *m_simplestation)//[by jim] need
{
#if 0
...
void display(m_simplestation_state *m_simplestation) //[by jim] need
{
  draw(m_simplestation, false, false, false);
  SDL_GL_SwapWindow(m_window);
}

void m_gpu_fill_rect(uint32_t m_value, m_simplestation_state *m_simplestation)//[by jim] need
{
}

void m_gpu_image_draw(uint32_t m_value, m_simplestation_state *m_simplestation)//[by jim] need
{
#if 1
    (void) m_value;
    ...
    uint16_t m_width = m_resolution & 0xFFFF;//[by jim] 320
 
    uint16_t m_height = m_resolution >> 16;//[by jim] 240
